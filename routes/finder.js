var express = require('express');
var router = express.Router();
var http = require('http-status-codes');

const db = require('../models/index.js');

const path = require('path')
const {spawn} = require('child_process')

const env = process.env.NODE_ENV || 'development';
const config = require(__dirname + '/../config/config.js')[env];

/**
 * Pet Finder endpoints
 */


// TODO: hay que evaluar aparte el caso en el que se crea una nueva mascota
// cuando se hace la busqueda, porque eso sería con una sola foto.
router.get('/:petId', async (req, res) => {
    /*const petPhotoEmbeddings = db.PetPhotos.findAll(
       { where: req.params.petId, attributes: [ 'embedding' ] }
    );*/
      

    if (config.use_env_variable) {
        //TODO: connection string case should be considered
        console.log(`Using ENV database ${config.use_env_variable}`);
        const connString = process.env[config.use_env_variable];
        throw new Error("Connection using database string not supported in finder module.")
    } 
    
    console.log(`Using config ${config}`)
    const databaseCredentials = { 
        host: config.host,
        port: 5432,
        db: config.database, 
        username: config.username, 
        pwd: config.password 
    };

    console.log("About to spawn process...");

    var process = spawn('python', [ 
        path.join(__dirname, '/python/classifier.py'),
        JSON.stringify(databaseCredentials),
        req.params.petId
     ]);

    console.log("Process spawned!");

    process.stderr.on('data', (data) => {
        console.error(`stderr: ${data}`);
    });
      
    // Takes stdout data from script which executed
    // with arguments and send this data to res object
    process.stdout.on('data', function(data) {
        return res.send(data.toString());
    });

});


module.exports = router;


    /**
    var inspect = require('inspect-stream');
    var arrayify = require('arrayify-merge.s');
    var slice    = require('slice-flow.s');
    const scikit = require('scikit-learn')

    const scikit = require('scikit-node')

    var features = scikit.dataset('load_digits.data');
    var labels   = scikit.dataset('load_digits.target');

    var trainingSet = arrayify();
    features.pipe(trainingSet);
    labels.pipe(trainingSet);

    var classifier = scikit.svm('SVC', {
        kernel: 'rbf',
        gamma: 'scale',
        C: 100
    });

    trainingSet
        .pipe(slice([0, -1])) //passes all packets except last one
        .pipe(classifier)
        .on('error', function (err) {
            console.log(err);
        })
        .on('end', function () {
            // now we have trained model

            var predict = classifier.predict();
            var features = scikit.dataset('load_digits.data');
            features.pipe(slice(-1)) //passes only last packet
            .pipe(predict)
            .pipe(inspect());
        });
     */

    /*

    scikit-learn does not seem to support svm package :(

    X = [[-1, -1], [-2, -1], [1, 1], [2, 1]]
    y = [1, 1, 2, 2]
    testX = [ [ 1, 0 ], [-3, -2] ]
    scikit.SKLearn(
        'svm', 
        [ 'SVC', 100, 'rbf' ],
        [
            [ 'fit', X, y ],
            [ 'predict_proba', testX ],
        ],
        (results) => {
            for (let i = 0; i < results.length; i++) {
                console.log(`Resulting prob prediction for ${testX[i]} was ${results[i]}`);
            }
        }
    );*/